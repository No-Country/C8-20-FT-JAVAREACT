...
const MyProvider = ({children}) =>{
    ...
    const total = () =>{
        if(get){
            return get.reduce((acc, product) => acc + product.price * product.qty)
        }
    }

    return(
        <Provider value={{get, cart, isInCart, setCart, addItem, deleteItem, emptyCart, quantity, qty, setQty, total}}>{children}</Provider>
    )
